...
`runtime-images.yml` 用于“发布运行态镜像”，不再做 SSH 远程部署。
...
1. 开发完成后 push 到 `main`
2. 等 `ci.yml` 通过
3. `runtime-images.yml` 会在 `main` push 后发布 GHCR 镜像（也可手动触发）
...
- 旧的 SSH + rsync 远程部署思路
- 在服务器上 `pnpm install` / `pnpm build` 的源码级部署路径

这些内容仅作历史背景参考，已经不再是推荐路径。
